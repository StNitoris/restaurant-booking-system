...
HttpResponse withCors(HttpResponse response) {
    ...
    ensureHeader(response, "Access-Control-Max-Age", "600");
    return response;
}

HttpResponse makePlainResponse(int status, std::string body) {
...
HttpResponse serveStaticFile(const std::string &root, const std::string &path) {
    ...
    if (!isSafePath(resolvedPath)) {
        return makePlainResponse(404, "Not Found");
    }
...
HttpResponse handleApiRequest(const HttpRequest &request,
    ...
    if (request.method == "GET" && isReservationIdPath(request.path)) {
        ...
        if (!reservation) {
            return makePlainResponse(404, "Reservation not found");
        }
    ...
    if (request.method == "POST" && request.path == "/api/reservations") {
        ...
        if (!hasField(data, "name") || !hasField(data, "phone") || !hasField(data, "partySize") ||
            !hasField(data, "time")) {
            return makePlainResponse(400, "Missing required fields");
        }
        auto partySizeOpt = toInt(*getFirstField(data, "partySize"));
        if (!partySizeOpt || *partySizeOpt <= 0) {
            return makePlainResponse(400, "Invalid party size");
        }
        auto timePoint = parseDateTime(*getFirstField(data, "time"));
        if (!timePoint) {
            return makePlainResponse(400, "Invalid time format");
        }
    ...
    if (request.method == "POST" && request.path == "/api/orders") {
        ...
        if (!hasField(data, "reservationId")) {
            return makePlainResponse(400, "Missing reservationId");
        }
        ...
        if (rawItems.empty()) {
            return makePlainResponse(400, "No items supplied");
        }
    ...
    if ((request.method == "PUT" || request.method == "DELETE") && isReservationIdPath(request.path)) {
        ...
        if (!durationMinutes || *durationMinutes <= 0) {
            return makePlainResponse(400, "Invalid duration");
        }
        ...
        if (tableSpecified) {
            ...
            if (tableField && !tableField->empty()) {
                ...
                if (!parsed || *parsed <= 0) {
                    return makePlainResponse(400, "Invalid table");
                }
        ...
        if (!sheet.updateReservationDetails(id,
                                            ...
                                            *timePoint,
                                            ...
                                            requestedTable,
                                            tableSpecified)) {
            return makePlainResponse(409, "Unable to update reservation");
        }
    ...
    if (request.method == "POST" && startsWith(request.path, statusPrefix) &&
        ...
        if (!hasField(data, "status")) {
            return makePlainResponse(400, "Missing status");
        }
        auto status = parseReservationStatus(*getFirstField(data, "status"));
        if (!status) {
            return makePlainResponse(400, "Invalid status");
        }
        auto reservation = sheet.findReservationById(id);
        if (!reservation) {
            return makePlainResponse(404, "Reservation not found");
...
void handleClient(int clientFd, Restaurant &restaurant, std::mutex &mutex, const std::string &staticRoot) {
    ...
    if (startsWith(request.path, "/api/")) {
        if (request.method == "OPTIONS") {
            ...
            response.contentType = "text/plain; charset=utf-8";
            response.body.clear();
            response = withCors(std::move(response));
        } else {
            response = withCors(handleApiRequest(request, restaurant, mutex));
        }
    } else {
