...
void handleClient(SocketHandle clientFd,
    ...
    if (request.method == "OPTIONS" && isApiRequest) {
        ...
        response = handleApiRequest(request, restaurant, mutex, onDataChanged);
        applyCorsHeaders(response, true);
    } else {
...
void runWebServer(Restaurant &restaurant,
    ...
    while (true) {
        ...
        std::thread worker(handleClient, clientFd, std::ref(restaurant), std::ref(mutex), staticDir, onDataChanged);
        worker.detach();
    }
}
