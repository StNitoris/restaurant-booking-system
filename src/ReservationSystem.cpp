...
void Reservation::restoreStatus(ReservationStatus status) { status_ = status; }

void Reservation::restoreTable(std::optional<int> tableId) { tableId_ = tableId; }

void Reservation::restoreLastModified(std::chrono::system_clock::time_point time) { lastModified_ = time; }
...
void BookingSheet::setDate(const std::string &date) { date_ = date; }

void BookingSheet::setNextReservationNumber(int next) {
    nextReservationNumber_ = std::max(1, next);
}

void BookingSheet::setNextOrderNumber(int next) { nextOrderNumber_ = std::max(1, next); }

int BookingSheet::getNextReservationNumber() const { return nextReservationNumber_; }

int BookingSheet::getNextOrderNumber() const { return nextOrderNumber_; }

void BookingSheet::replaceState(std::string date,
    ...
    setNextReservationNumber(nextReservationNumber);
